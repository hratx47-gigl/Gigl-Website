...
function postApplyToGig(req, res) {
  ...
  Gig.findOneAndUpdate({ _id: gigId }, { $addToSet: { applicants: user } })
    .then((response) => {
      res.send(response);
    })
    .catch((err) => {
...
function getPerformerProfileInfo(req, res) {
  const user = req.session.userPerformer;
  UserPerformer.findById(
    user._id,
    "about location education experience username"
  )
    .exec()
    .then((docs) => {
      res.json(docs);
...
function putPerformerProfileInfo(req, res) {
  ...
  UserPerformer.findByIdAndUpdate(user._id, { [field]: newText })
    .exec()
    .then((result) => {
...
module.exports = {
  getAllGigs,
  getPerformerAcceptedGigs,
  getPerformerPendingGigs,
  getPerformerAvailableGigs,
  getPerformerProfileInfo,
  putPerformerProfileInfo,
  postApplyToGig,
};
